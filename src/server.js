...
window.printSelected=()=>{refreshPrintCommand();if(!bridgeAvailable()){onPrintResult(false,'Newland EB JS Bridge not available');return}const type=q('#printerType')?.value||'TCP';const address=(q('#printer')?.value||'').trim();const lang=q('#language')?.value||'CPCL';if(!address){onPrintResult(false,type==='TCP'?'Enter printer address such as 192.168.1.100:9100':'Enter Bluetooth MAC address');return}if(type!=='TCP'){onPrintResult(false,'Bluetooth SPP is not enabled in this Newland EB build yet');return}const ok=typeof Android.printRawTo==='function'?Android.printRawTo(type,address,lang,q('#command').value):Android.printZplTo(address,q('#command').value);if(!ok)onPrintResult(false,'Print command was rejected')};
