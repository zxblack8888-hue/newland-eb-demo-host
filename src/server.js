...
window.scanBluetoothPrinters=()=>{const st=q('#btDialogStatus');if(st)st.textContent='Scanning nearby Bluetooth printers...';if(!(window.Android&&typeof Android.startBluetoothPrinterScan==='function')){onBluetoothPrinterScan(false,'Newland EB Bluetooth scan Bridge not available',btDevices);return}const ok=Android.startBluetoothPrinterScan(12000);if(!ok&&st)st.textContent='Waiting for Bluetooth permission or scan startup...'};
...
window.onBluetoothPrinterScan=(scanning,error,devices)=>{renderBluetoothPrinters(Array.isArray(devices)?devices:btDevices);const message=error?'Bluetooth scan: '+error:(scanning?'Scanning nearby Bluetooth printers...':'Bluetooth scan complete. Select a printer and tap Connect.');const st=q('#btDialogStatus');if(st){st.className=error?'warn mono tiny':'mono tiny';st.textContent=message}const el=q('#printResult');if(el){el.className=error?'warn mono tiny':'mono tiny';el.textContent=message}};
