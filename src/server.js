...
function css() {
  return `:root{font-family:Arial,Helvetica,sans-serif;color:#edf3f8;background:#101418}body{margin:0;background:#101418}header{display:flex;justify-content:space-between;gap:16px;align-items:center;padding:16px 18px;background:#17202b;border-bottom:1px solid #273244}.brand{font-size:22px;font-weight:700}.status{color:#7dd3fc;font-size:14px}.screen{padding:18px;max-width:760px;margin:0 auto}.panel{border:1px solid #2c3848;border-radius:8px;background:#151b24;padding:16px;margin-bottom:14px}.title{font-size:24px;font-weight:800;color:#facc15;margin:0 0 14px}.row{margin:14px 0}label{display:block;color:#facc15;font-size:13px;font-weight:700;text-transform:uppercase;margin-bottom:7px}input,select,textarea{box-sizing:border-box;width:100%;font-size:22px;padding:14px 16px;border:2px solid #334155;border-radius:8px;background:#0f141c;color:#f8fafc}textarea{font-size:13px;min-height:168px;line-height:1.35}input:focus,textarea:focus{outline:none;border-color:#facc15}.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:16px}button{font-size:18px;font-weight:700;padding:12px 14px;border:0;border-radius:8px;background:#2563eb;color:white}button.secondary{background:#334155}.ok{color:#4ade80}.warn{color:#facc15}.mono{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}.tiny{font-size:12px;line-height:1.25}.zoomed .tiny{font-size:22px;line-height:1.38}.zoomed .mono{font-size:20px;white-space:normal;word-break:break-word}.modal{position:fixed;inset:0;background:rgba(2,6,23,.74);display:flex;align-items:flex-end;justify-content:center;padding:14px;z-index:20}.modal-card{box-sizing:border-box;width:100%;max-width:720px;height:88vh;max-height:760px;display:flex;flex-direction:column;overflow:hidden;border:1px solid rgba(147,197,253,.45);border-radius:16px;background:rgba(15,23,42,.96);padding:16px;box-shadow:0 18px 60px rgba(0,0,0,.5)}.modal-title,.modal-card>.tiny,.scan-row,#btDialogStatus{flex:0 0 auto}.modal-title{font-size:22px;font-weight:800;color:#facc15;margin:0 0 8px}.modal-actions{flex:0 0 auto;display:flex;gap:10px;flex-wrap:wrap;margin-top:12px;padding-top:12px;border-top:1px solid rgba(148,163,184,.24);background:rgba(15,23,42,.96)}.scan-row{display:flex;align-items:center;gap:10px;margin:10px 0 12px}.scan-dot{width:12px;height:12px;border-radius:50%;background:#64748b}.scanning .scan-dot{background:#38bdf8;animation:pulse 900ms infinite}.scan-wave{height:6px;flex:1;border-radius:999px;background:linear-gradient(90deg,#1e293b,#38bdf8,#1e293b);background-size:220% 100%}.scanning .scan-wave{animation:sweep 1.1s linear infinite}.device-section{color:#93c5fd;font-size:12px;font-weight:800;margin:12px 0 4px;text-transform:uppercase}.device-list{flex:1 1 0;height:0;min-height:0;overflow:auto;display:grid;gap:8px;margin:12px 0;padding-right:2px}.device-card{width:100%;text-align:left;background:rgba(30,41,59,.86);border:1px solid rgba(148,163,184,.35);border-radius:10px;padding:12px;color:#e5edf7}.device-card.selected{border-color:#facc15;background:rgba(37,99,235,.7)}.device-card.nearby{border-color:rgba(56,189,248,.75)}.device-name{font-size:17px;font-weight:800}.device-meta{font-size:12px;color:#cbd5e1;margin-top:4px}@keyframes pulse{0%,100%{transform:scale(.8);opacity:.55}50%{transform:scale(1.35);opacity:1}}@keyframes sweep{0%{background-position:0 0}100%{background-position:220% 0}}nav{display:flex;flex-wrap:wrap;gap:6px;padding:10px 18px 16px;background:#0b1017;border-top:1px solid #273244}nav button{flex:1 1 30%;font-size:13px;padding:10px 6px;background:#17202b}`;
}
...
window.renderBluetoothPrinters=(devices)=>{btDevices=Array.isArray(devices)?devices:[];const sel=q('#btPrinter');if(sel){if(!btDevices.length){sel.innerHTML='<option value="">No Bluetooth printers found yet</option>'}else{sel.innerHTML=btDevices.map(d=>'<option value="'+esc(d.address)+'">'+esc((d.name||d.address)+' · '+d.address+(d.scanType?' · '+d.scanType.toUpperCase():'')+(d.bonded?' · paired':'')+(d.supportsSpp?' · SPP':' · SPP unknown'))+'</option>').join('');if(btSelected)sel.value=btSelected}}renderBluetoothDeviceList()};
...
window.openBluetoothPrinterDialog=()=>{let modal=q('#btModal');if(!modal){modal=document.createElement('div');modal.id='btModal';modal.className='modal';document.body.appendChild(modal)}modal.innerHTML='<div class="modal-card"><h2 class="modal-title">Bluetooth Printer Setup</h2><p class="tiny">Turn on the printer, make it discoverable, then scan. Nearby devices appear separately from known paired devices. If Android asks for a PIN, enter the printer PIN such as 1234 or 0000.</p><div id="btScanVisual" class="scan-row"><span class="scan-dot"></span><span class="scan-wave"></span></div><p id="btDialogStatus" class="mono tiny">Ready to scan.</p><div id="btDeviceList" class="device-list"></div><div class="modal-actions"><button type="button" onclick="scanBluetoothPrinters()">Scan Nearby</button><button type="button" onclick="connectSelectedBluetoothPrinter()">Connect</button><button type="button" class="secondary" onclick="stopBluetoothPrinterScan()">Stop</button><button type="button" class="secondary" onclick="closeBluetoothPrinterDialog()">Close</button></div></div>';refreshBluetoothPrinters();scanBluetoothPrinters()};
window.closeBluetoothPrinterDialog=()=>{stopBluetoothPrinterScan();q('#btModal')?.remove()};
window.renderBluetoothDeviceList=()=>{const list=q('#btDeviceList');if(!list)return;if(!btDevices.length){list.innerHTML='<p class="warn tiny">No printers found yet. Keep the printer powered on and discoverable, then tap Scan Nearby.</p>';return}const nearby=btDevices.filter(d=>d.discovered);const paired=btDevices.filter(d=>!d.discovered);const render=(title,items)=>items.length?'<div class="device-section">'+title+'</div>'+items.map(d=>'<button type="button" class="device-card '+(d.discovered?'nearby ':'')+(d.address===btSelected?'selected':'')+'" onclick="selectBluetoothPrinter(\\''+esc(d.address)+'\\')"><div class="device-name">'+esc(d.name||d.address)+'</div><div class="device-meta">'+esc(d.address)+' · '+(d.scanType?d.scanType.toUpperCase():'known')+' · '+(d.bonded?'paired':'not paired')+' · '+(d.supportsSpp?'SPP':'SPP unknown')+'</div></button>').join(''):'';list.innerHTML=render('Nearby scan results',nearby)+render('Known paired devices',paired)};
window.selectBluetoothPrinter=(address)=>{btSelected=address;const sel=q('#btPrinter');if(sel)sel.value=address;renderBluetoothDeviceList();const st=q('#btDialogStatus');if(st)st.textContent='Selected MAC: '+address};
window.scanBluetoothPrinters=()=>{const st=q('#btDialogStatus');const visual=q('#btScanVisual');if(st)st.textContent='Scanning nearby Bluetooth printers...';if(visual)visual.classList.add('scanning');if(!(window.Android&&typeof Android.startBluetoothPrinterScan==='function')){onBluetoothPrinterScan(false,'Newland EB Bluetooth scan Bridge not available',btDevices);return}const ok=Android.startBluetoothPrinterScan(15000);if(!ok&&st)st.textContent='Waiting for Bluetooth permission, Bluetooth enable, or scan startup...'};
...
window.onBluetoothPrinterScan=(scanning,error,devices)=>{renderBluetoothPrinters(Array.isArray(devices)?devices:btDevices);const visual=q('#btScanVisual');if(visual)visual.classList.toggle('scanning',!!scanning);const nearby=btDevices.filter(d=>d.discovered).length;const paired=btDevices.filter(d=>!d.discovered).length;const message=error?'Bluetooth scan: '+error:(scanning?'Scanning nearby Bluetooth printers... Found '+nearby+' nearby, '+paired+' paired.':'Bluetooth scan complete. Found '+nearby+' nearby, '+paired+' paired. Select a printer and tap Connect.');const st=q('#btDialogStatus');if(st){st.className=error?'warn mono tiny':'mono tiny';st.textContent=message}const el=q('#printResult');if(el){el.className=error?'warn mono tiny':'mono tiny';el.textContent=message}};
window.connectSelectedBluetoothPrinter=()=>testBluetoothPrinter();
window.testBluetoothPrinter=()=>{const address=btSelected||q('#btPrinter')?.value||'';if(!address){onBluetoothPrinterResult(false,'Scan and select a Bluetooth printer first','');return}btSelected=address;if(!(window.Android&&typeof Android.testBluetoothPrinter==='function')){onBluetoothPrinterResult(false,'Newland EB Bluetooth Bridge not available',address);return}const st=q('#btDialogStatus');if(st)st.textContent='Connecting to '+address+' ...';const ok=Android.testBluetoothPrinter(address);if(!ok)onBluetoothPrinterResult(false,'Connect test was rejected',address)};
window.onBluetoothPrinterResult=(success,error,address)=>{if(address){btSelected=address;const sel=q('#btPrinter');if(sel)sel.value=address}const message=success?'Bluetooth SPP connected. MAC: '+address:'Bluetooth connect: '+(error||'unknown error');const st=q('#btDialogStatus');if(st){st.className=success?'ok mono tiny':'warn mono tiny';st.textContent=message}const el=q('#printResult');if(el){el.className=success?'ok mono tiny':'warn mono tiny';el.textContent=message}renderBluetoothDeviceList()};
