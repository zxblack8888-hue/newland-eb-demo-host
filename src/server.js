...
function bridgeAvailable(){return !!(window.Android&&typeof Android.printZpl==='function')}
function demoZpl(labelId,item,qty){return '^XA\\n^CI28\\n^FO40,35^A0N,36,36^FDNewland EB Print Test^FS\\n^FO40,90^BY2\\n^BCN,90,Y,N,N\\n^FD'+labelId+'^FS\\n^FO40,220^A0N,32,32^FDItem: '+item+'^FS\\n^FO40,265^A0N,32,32^FDQty: '+qty+'^FS\\n^FO40,320^A0N,24,24^FDRailway demo host / JS Bridge^FS\\n^XZ'}
...
window.printTest=()=>{const id='LBL-'+Date.now().toString().slice(-6);const z=demoZpl(id,'WIDGET-001','12');page('Print Test','<p>This page validates Newland EB JS Bridge printing. It sends ZPL to the configured default printer or to the address below.</p><p id="bridgeStatus" class="'+(bridgeAvailable()?'ok':'warn')+'">'+(bridgeAvailable()?'Newland EB Bridge detected':'Open this page inside Newland EB to print')+'</p>'+field('printer','Printer address optional','text',false,'')+field('labelId','Label ID','text',true,id)+field('item','Item','text',true,'WIDGET-001')+field('qty','Quantity','number',false,'12')+'<div class="row"><label for="zpl">ZPL Preview</label><textarea id="zpl" class="mono">'+esc(z)+'</textarea></div><p id="printResult" class="mono tiny">Waiting for print action.</p><div class="actions"><button onclick="refreshZpl()">Refresh ZPL</button><button onclick="printDefault()">Print Default</button><button class="secondary" onclick="printAddress()">Print Address</button></div>')};
window.refreshZpl=()=>{const z=demoZpl(q('#labelId')?.value||'LBL-DEMO',q('#item')?.value||'WIDGET-001',q('#qty')?.value||'1');const el=q('#zpl');if(el)el.value=z};
...
window.printDefault=()=>{refreshZpl();if(!bridgeAvailable()){onPrintResult(false,'Newland EB JS Bridge not available');return}const ok=Android.printZpl(q('#zpl').value);if(!ok)onPrintResult(false,'Print command was rejected')};
window.printAddress=()=>{refreshZpl();if(!bridgeAvailable()){onPrintResult(false,'Newland EB JS Bridge not available');return}const address=(q('#printer')?.value||'').trim();if(!address){onPrintResult(false,'Enter printer address such as 192.168.1.100:9100');return}const ok=Android.printZplTo(address,q('#zpl').value);if(!ok)onPrintResult(false,'Print command was rejected')};
